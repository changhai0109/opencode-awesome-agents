...
Three tiers, each a POOL of equivalent models — `subagent_dispatch` picks one
at random, briefly probes it for usability with the internal `ping` agent,
and falls back to the rest (random order) on failure:
...
- **plus** — standard implementation, research, review:
  `openai/gpt-5.6-terra`, `kimi-code-plan-cn/kimi-for-coding`
- **regular** — mechanical work and needle queries: `openai/gpt-5.6-luna`,
  `minimax-cn-coding-plan/MiniMax-M3`

Claude (`claude-code/...`) models are excluded from every pool: sessions on
that provider cannot see custom plugin tools, so a spawn-capable agent
landing there could not sub-delegate. Only use them via an explicit `models`
list when the user asks for Claude.
...
(`premium`/`plus`/`regular` — draws from that tier's model pool) or `models`
(explicit `provider/model` pool). One candidate is picked at random, probed
for usability by default (`probe: false` skips it), and the rest are
fallbacks. `spawn_budget` (the child's subtree budget, default 0 = leaf) is
ENFORCED by the tool — an over-budget call is refused, so pass the same
number in the arg and in the task header.

- Every dispatch MUST name a tier (or an explicit models list). Prefer `tier`
  over `models`: the tier pools already encode which models are equivalent
  for a job. Reach for `models` only for special cases, e.g. a second opinion
